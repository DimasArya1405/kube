...
<div class="mb-8">
    ...
    <p class="text-gray-500 mt-1">Berikut data hasil prediksi KUBE yang telah dilakukan.</p>
</div>
...
<div class="flex justify-between items-center mb-5">
    <a href="{{ route('prediksi.index') }}"
       class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow transition">
        <span>+</span>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
            <form method="GET" action="{{ route('prediksi.daftar') }}">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('prediksi.detail', $item->id_prediksi) }}"
                                   class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition"
                                   title="Lihat Detail">
                                    👁
                                </a>

                                <a href="{{ route('prediksi.edit', $item->id_prediksi) }}"
                                   class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-yellow-50 text-yellow-600 hover:bg-yellow-100 transition"
                                   title="Edit">
                                    ✏️
                                </a>

                                <a href="{{ route('prediksi.track', [$item->id_kube, $item->tahun]) }}"
