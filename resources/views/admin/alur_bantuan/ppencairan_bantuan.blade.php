...
@section('content')
    ...
    <div class="bg-white mb-6 rounded-lg shadow-sm border overflow-hidden">
        <div class="bg-white p-4 mb-6 rounded-lg shadow-sm border">
    <form action="{{ route('admin.pencairan_bantuan.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tahun Pengajuan</label>
        ...
        <div class="flex gap-2">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm transition">
                Filter
            </button>
            @if(request('tahun') || request('status'))
                <a href="{{ route('admin.pencairan_bantuan.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm transition">
                    Reset
                </a>
            @endif
        </div>
                                ...
                                @if ($row->status_pencairan == 'menunggu')
                                    <div class="flex gap-2">
                                        <a href="{{ route('admin.pencairan_bantuan.accept', $row->id_pencairan) }}"
                                            onclick="return confirm('Apakah anda yakin menyetujui pencairan {{ $row->pengajuan_kube?->kube?->nama_kube ?? }}?')"
                                        ...
                                        <a href="{{ route('admin.pencairan_bantuan.reject', $row->id_pencairan) }}"
                                            onclick="return confirm('Apakah anda yakin menolak pencairan {{ $row->pengajuan_kube?->kube?->nama_kube ??}}?')"
